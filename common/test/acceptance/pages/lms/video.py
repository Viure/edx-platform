...
# Maximum number of seconds to wait for the `Promise` to be satisfied before timing out.
WAIT_TIMEOUT = 200

# Number of attempts to make to satisfy the `Promise`.  Can be `None` to disable the limit.
WAIT_TRY_LIMIT = None
...
class VideoPage(PageObject):
    ...
    def _wait_for_element(self, element_css_selector, promise_desc, try_limit=WAIT_TRY_LIMIT, timeout=WAIT_TIMEOUT,
        ...
        :param try_limit (int or None): Number of attempts to make to satisfy the `Promise`. Can be `None` to disable
               the limit.
        :param timeout (float): Number of seconds to wait between attempts.
        :param try_interval (float): Maximum number of seconds to wait for the `Promise` to be satisfied before timing
               out.
